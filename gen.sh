...
# 2. 生成一个用于服务器的私钥和CSR证书请求
openssl req -newkey rsa:4096 -keyout server-key.pem -out server-req.pem -nodes -subj "/C=CN/ST=Shaanxi/L=Xian/O=raja/OU=raja/CN=raja/emailAddress=1647193241@qq.com"
echo "服务器私钥和CSR证书请求生成完毕"

# 3. 使用CA的私钥去注册服务器CSR，以及返回注册的证书
